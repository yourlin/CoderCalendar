...
angular.module('starter', ['ionic'])
    ...
    .controller('CalendarController', function ($scope, $http) {
        var today = new Date();
        var data = getCodeCalendar(today);

        $scope.goodList = data.goodList;
        $scope.badList = data.badList;
        $scope.direction = data.direction;
        $scope.drinks = data.drinks;
        $scope.encounter = star(data.encounter);
        $scope.getTodayString = getTodayString;
    });

//获得数据
function getCodeCalendar(theDay) {
    var randomFactor = theDay.getFullYear() * 10000 + (theDay.getMonth() + 1) * 100 + theDay.getDate();
    var data = {
        goodList: [],
        badList: [],
        direction: directions[random(randomFactor, 2) % directions.length],
        drinks: pickRandom(randomFactor, drinks, 2).join('，'),
        encounter: random(randomFactor, 6) % 5 + 1
    };

    var _activities = filter(theDay, activities);
    var numGood = random(randomFactor, 98) % 3 + 2;
    var numBad = random(randomFactor, 87) % 3 + 2;
    var eventArr = pickRandomActivity(randomFactor, _activities, numGood + numBad);

    for (var i = 0; i < numGood; i++) {
        var good = {};
        good.title = eventArr[i].name;
        good.description = eventArr[i].good;
        data.goodList.push(good);
    }

    for (i = numGood; i < numGood + numBad; i++) {
        var bad = {};
        bad.title = eventArr[i].name;
        bad.description = eventArr[i].bad;
        data.badList.push(bad);
    }
...
// 去掉一些不合今日的事件
function filter(theDay, activities) {
    var result = [];
...
function isWeekend(theDay) {
    return theDay.getDay() == 0 || theDay.getDay() == 6;
}

// 从 activities 中随机挑选 size 个
function pickRandomActivity(randomFactor, activities, size) {
    var picked_events = pickRandom(randomFactor, activities, size);

    for (var i = 0; i < picked_events.length; i++) {
        picked_events[i] = parse(randomFactor, picked_events[i]);
    }
...
// 从数组中随机挑选 size 个
function pickRandom(randomFactor, array, size) {
    var result = [];
    ...
    for (var j = 0; j < array.length - size; j++) {
        var index = random(randomFactor, j) % result.length;
...
// 解析占位符并替换成随机内容
function parse(theDay, event) {
    var result = {name: event.name, good: event.good, bad: event.bad};  // clone

    if (result.name.indexOf('%v') != -1) {
        result.name = result.name.replace('%v', varNames[random(theDay, 12) % varNames.length]);
    }

    if (result.name.indexOf('%t') != -1) {
        result.name = result.name.replace('%t', tools[random(theDay, 11) % tools.length]);
    }

    if (result.name.indexOf('%l') != -1) {
        result.name = result.name.replace('%l', (random(theDay, 12) % 247 + 30).toString());
    }
